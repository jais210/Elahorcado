...
//1.Mostrar al usuario la palabra oculta (solo primera letra y lo demás en asteriscos)
...
function palabraOculta(palabra) {
  ...
  console.log(primeraLetra + otrasLetras);
}
palabraOculta(palabra);

//2.El usuario debe ingresar una letra por cada asterisco.

var usuario = "l";

function juego(usuario, palabra){
  
  for (i=[1];i<palabra.length; i++){
    if(usuario == palabra[1]){
      console.log(palabra[1]);
    } else {console.log(imprimirMatriz(hombre[0]))}
  }
} 
